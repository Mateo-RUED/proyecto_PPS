...
"👉 Qué hace    :
    ...
    Esto ajusta los conteos para tener en cuenta las diferencias en profundidad de secuenciación
    Es un paso crucial para comparar muestras de manera justa
    primero se debe normalizar a partir de las musgtras d eesta manera no cometemos el error
    de aplizar VST antes ya que pueden haber muestras que secuenciaron mas lecturas
...
"👉 Qué hace    :
    ...
    de esta manera no cometemos el error de aplicar VST antes ya que pueden haber muestras que secuenciaron mas lecturas
    que otras y eso nos dan errores en la varianza estabilizada.

    De esta forma todos los genes tienen una varianza casi constante, lo que facilita la comparación entre ellos
